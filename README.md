...
4. **Environment Configuration (Optional)**:
   Create a `.env` file in `capitalguard/backend/.env`:
   ```env
...
4. Vercel will automatically read [vercel.json](file:///c:/Users/Vivek/OneDrive/Desktop/vivek/vercel.json) and [package.json](file:///c:/Users/Vivek/OneDrive/Desktop/vivek/package.json), install dependencies, and build the frontend with SPA routing rewrites.
5. Click **Deploy**.

### Option B: Deploy with Subdirectory Root
If you prefer setting the Root Directory in Vercel:
1. In Vercel Project Settings, set **Root Directory** to `capitalguard/frontend`.
2. Framework Preset: **Vite**.
3. Build Command: `npm run build` (or `tsc -b && vite build`).
4. Output Directory: `dist`.
5. The included [capitalguard/frontend/vercel.json](file:///c:/Users/Vivek/OneDrive/Desktop/vivek/capitalguard/frontend/vercel.json) handles client-side routing rewrites for `/risk`, `/alerts`, `/scenarios`, and `/audit`.

### Environment Variables on Vercel (Optional)
If you have deployed the FastAPI backend (e.g. on Railway, Render, Fly.io, or AWS), configure these variables in Vercel **Settings > Environment Variables**:
